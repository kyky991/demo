...
public class TestRowCount {
    public static void main(String[] args) throws Throwable {
        ...
        Map<byte[], GetRowCount.getRowCountResponse> results = null;
        ...
        try {
            ...
            // 调用 RPC，并对返回值进行处理。
            Batch.Call<GetRowCount.hbaseEndPointTestService, GetRowCount.getRowCountResponse> callable = new Batch.Call<GetRowCount.hbaseEndPointTestService, GetRowCount.getRowCountResponse>() {
                ServerRpcController controller = new ServerRpcController();

                // 定义返回
                BlockingRpcCallback<GetRowCount.getRowCountResponse> rpcCallback = new BlockingRpcCallback<GetRowCount.getRowCountResponse>();

                // 下面重载 call 方法，API会连接到region后会运行call方法来执行服务的请求
                @Override
                public GetRowCount.getRowCountResponse call(GetRowCount.hbaseEndPointTestService instance) throws IOException {
             ...
             */
            results = table.coprocessorService(GetRowCount.hbaseEndPointTestService.class,
                    null, null,
                    callable);

            // 合并多个Region的结果
            Collection<GetRowCount.getRowCountResponse> resultsc = results.values();
            long result = 0L;
            for (GetRowCount.getRowCountResponse r : resultsc) {
